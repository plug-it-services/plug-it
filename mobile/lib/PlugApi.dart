...
class PlugApi {
  ...
  static void handleError(Response result) {
    ...
    if (result.data == null) {
      throw PlugError(
          ...
          statusCode: 500,
          statusMessage:
              "Error occurred but not raised by server, no data received.");
    }
  }
  ...
  static Future<Event?> getEvent(String serviceName, String eventId,
      {bool isTrigger = true}) async {
  ...
  static Future OAuth2(String serviceName) async {
    Response result = await dio.post("$apiUrl/service/$serviceName/oauth2",
        data: {'redirectUrl': 'plugit://app'}, options: getHeaders());
    String authUrl = result.data['url'] ?? "";

    return launchUrl(Uri.parse(authUrl), mode: LaunchMode.externalApplication);
  }

  static Future OAuthApiKey(String serviceName, String apiKey) async {
    return dio.post("$apiUrl/service/$serviceName/apiKey",
        data: {'apiKey': apiKey}, options: getHeaders());
  }

  static Future OAuthCredentials(
      String serviceName, String id, String secret) async {
    return dio.post("$apiUrl/service/$serviceName/clientSecrets",
        data: {'clientId': id, 'clientSecret': secret}, options: getHeaders());
  }
  ...
  static Future<List<Event>?> getServiceEvents(String serviceName) async {
    ...
    Response result = await dio.get("$apiUrl/service/$serviceName/events",
        options: getHeaders());
    handleError(result);
    events = [];
    for (var json in result.data!) {
  ...
  static Future<List<Event>?> getServiceActions(String serviceName) async {
    ...
    Response result = await dio.get("$apiUrl/service/$serviceName/actions",
        options: getHeaders());
    handleError(result);
    events = [];
    for (var json in result.data!) {
  ...
  static Future<bool> createPlug(PlugDetails plug) async {
    Response result =
        await dio.post("$apiUrl/plugs", data: plug, options: getHeaders());
    return handleResponseCheck(result, excpeted: 201);
  }
  ...
  static Future<bool> deletePlug(String id) async {
    Response result =
        await dio.delete("$apiUrl/plugs/$id", options: getHeaders());
    return handleResponseCheck(result);
  }

  static Future<bool> editPlug(PlugDetails plug) async {
    Response result = await dio.put("$apiUrl/plugs/${plug.id}",
        data: plug, options: getHeaders());
    return handleResponseCheck(result);
  }

  static Future<bool> enablePlug(String id, bool enabled) async {
    Response result = await dio.put("$apiUrl/plugs/$id/enabled",
        queryParameters: {'enabled': enabled.toString().toLowerCase()},
        options: getHeaders());
    return handleResponseCheck(result);
  }
  ...
  static Future<bool> login(String email, String password) async {
    ...
    Response result = await dio.post("$apiUrl/auth/login",
  ...
  static Future<bool> register(
    ...
    Response result = await dio.post("$apiUrl/auth/signUp",
        data: {
          "firstname": firstName,
          "lastname": lastName,
          "email": email,
  ...
  static Future<bool> ssoLogin(String serviceName, String code) async {
    Response result = await dio.post("$apiUrl/auth/$serviceName/login",
        data: {"code": code, "codeType": "accessToken"}, options: getHeaders());
    token = result.headers.map['Set-Cookie']?[0];
    return handleResponseCheck(result);
  }
}
